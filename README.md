...
| Requirement | Version |
|---|---|
| Python | **3.12.10** (PyTorch is not yet compatible with 3.13/3.14) |
| Node.js | 18+ |
...
# 2. Install dependencies (en_core_web_sm is installed automatically, pinned in requirements.txt)
...
- **Stub rejection** — answers under 40 characters, or containing placeholder phrases ("answer here", "refer to above", etc.), are rejected at ingestion and flagged `verified=False`.
- **Hard session filter** — question selection only queries `verified=True AND ideal_answer IS NOT NULL AND code_expected=False`. Unverified or code-implementation questions never appear in voice sessions.
...
|---|---|---|
| Repeat the question back | ≤ 18 (parroting guard caps hard at 18 if ≥ 70% token overlap with the question) | ≤ 32 (substance ≈ 0) |
...
- **Parroting detection** (`completeness_score.py`) — `_parroting_ratio()` measures the fraction of the answer's meaningful tokens that overlap with the question's. At ≥70% overlap the score is hard-capped at 18; between 55–70% it's scaled down gradually.
- **Substance score** (`confidence_scoring.py`) — `_substance_score()` measures original content (words not in the question and not stop words) as part of the grammar sub-score. Parroting drives substance towards 0.
- **Word-count gate** (`confidence_scoring.py`) — answers under 8 words are hard-capped at 28 confidence; 8–15 words are capped linearly up to 50.
...
- **STAR framework** — behavioral answers are evaluated on 4 components (Situation, Task, Action, Result) via 57 trigger phrases; missing components lower the completeness score.
...
The app is currently deployed on free-tier hosting. Free tiers have limited RAM, storage, and uptime, so performance and availability may be upgraded to paid infrastructure as usage grows.
...
A migration script (`scripts/fix_session_scores.py`) was created to fix this inconsistency:

```powershell
cd interview_coach
.\venv_py312\Scripts\python.exe scripts\fix_session_scores.py
```
...
| `DATABASE_URL is None` | `.env` file not found | Ensure `interview_coach/.env` exists with a valid `DATABASE_URL` |
